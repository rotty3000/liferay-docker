...
function prepare_mount {
	...
	then
		...
		if [ "$(uname)" == "Darwin" ]
		then
			local patching_tool_file_name=$(find downloads/patching-tool/ -maxdepth 1 -name '*.zip' -print0 | xargs -0 stat -f '%T+\t%f\n' | sort | tail -n 1 | awk '{print $2}')
		else
			local patching_tool_file_name=$(find downloads/patching-tool/ -maxdepth 1 -name '*.zip' -printf "%T+\t%f\n" | sort | tail -n 1 | awk '{print $2}')
		fi
	fi
